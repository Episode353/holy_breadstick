...
void displayGrid(ViewableArea area) {
    ...
    // Set the NPC's positions and symbols in the grid
    for (int i = 0; i < numNPCs; i++) {
        int x = npcs[i].x - x_loc + area.width / 2;
        int y = npcs[i].y - y_loc + area.height / 2;
        if (x >= 0 && x < area.width && y >= 0 && y < area.height) {
            grid[y][x] = npcs[i].display_char;
        }
    }
    ...
    for (int i = area.height - 1; i >= 0; i--) {
        printf("|"); // Print the left border
            ...
            for (int j = 0; j < area.width; j++) {
                // Get the color and symbol of the item or wall at the current position (j, i)
                char color[20] = "";
                char symbol = grid[i][j];
                int foundItem = 0; // Flag to indicate if an item is found at the current position
                int foundWall = 0; // Flag to indicate if a wall is found at the current position
            ...
            if (j == playerX && i == playerY) {
                ...
                // If no item is found, check if there is an NPC at the current position
                if (!foundItem) {
                    for (int k = 0; k < numNPCs; k++) {
                        int x = npcs[k].x - x_loc + area.width / 2;
                        int y = npcs[k].y - y_loc + area.height / 2;
                        if (x == j && y == i) {
                            strcpy(color, npcs[k].color); // Set NPC color
                            symbol = npcs[k].display_char; // Set NPC symbol
                            foundItem = 1;
                            break;
                        }
                    }
