...
void displayGrid(ViewableArea area) {
    ...
    for (int i = y_loc + area.height - 1; i >= y_loc; i--) {
        printf("|"); // Print the left border

        for (int j = x_loc; j < x_loc + area.width; j++) {
            ...
            if (j == x_loc && i == y_loc) {
                ...
                // If no item is found, check if there isa biome at the current position
                if (!foundItem) {
...
int main() {
    ...
    // Display the grid with the viewable area
    displayGrid(viewableArea);
